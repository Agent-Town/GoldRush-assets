...
Blender --background --python this.py. Existing raster sources are copied byte-for-byte.
...
source=ROOT/'assets/raw/archive-world-floor-material-v1.png';shutil.copy2(source,OUT/'last-claim-terrain-atlas.png')
shutil.copy2(OUT/'archive-world-panorama-atlas.png',OUT/'last-claim-panorama-atlas.png')
...
def facts(o,path,budget):
 ...
 return {'meshCount':1,'primitiveCount':1,'materialCount':1,'vertices':len(verts),'triangles':tri,'triangleBudget':budget,'boundsMeters':{'min':[min(v[i] for v in verts) for i in range(3)],'max':[max(v[i] for v in verts) for i in range(3)]},'sha256':sha(path)}
...
terrain=mesh('LastClaimTerrain',verts,faces,terrainMat,lambda co:((co.x+64)/12,(co.y+64)/12),True)
...
for poly in panorama.data.polygons:
 ...
 for corner,li in enumerate(poly.loop_indices):
  ...
  uv.data[li].uv=(u,[.02,.045,.075,.13,1.0][rr])
...
terrainContract={'asset':'last-claim-terrain.glb','contractId':'e10-last-claim','tileId':'ark-plaza-e10','renderOnly':True,'simulation':'Planar and unchanged; no collision, deck-loss, preserve, spawn or build rule is altered.','heightSocket':'Terrain.visualY','theme':'Circular brass-inlaid memorial deck across the full original square floor; no walkable corner is removed.',**terrainFacts,'texture':texture(terrainImage),'maskTable':'assets/contracts/epoch-10-deepsky/mask-tables/e10-last-claim.json',**truth,'panoramaMount':panoMount,'landmarkMounts':mounts,'landmarkAcceptanceStations':stations,'sourceArt':['assets/raw/plate-contract-e10-last-claim.png','assets/raw/archive-world-floor-material-v1.png'],'heightDerivation':{'grid':'last-claim-fallback-height-grid.json','sha256':sha(OUT/'last-claim-fallback-height-grid.json'),'method':'16641 fallback visual samples; unchanged sampler code and planar gameplay.'},'files':files('last-claim-terrain')}
# Contract bounds use Blender x/y/z; symmetric XZ extents preserve the established convention.
panoContract={'asset':'last-claim-panorama.glb','map':'last-claim','renderOnly':True,'style':'Separate star wall beyond a lowered stone apron, within Panorama Law v2 budget.',**panoFacts,'texture':texture(panoImage),'mount':panoMount,'projection':{'skyRingRadiusMeters':190,'skyBottomMeters':-5,'skyTopMeters':115,'groundSkirtInnerBoundaryMeters':{'shape':'expanded-playfield-rectangle','halfExtents':[64,64],'margin':.05}},'sourceArt':['assets/pilots/map-rebuild-spike/archive-world-panorama-atlas.png'],'files':files('last-claim-panorama')}

...
REEXPORT = Path("/tmp/old-digger-reexport.glb")
...
shared = load("old_digger_shared_verify", ROOT / "assets/pilots/dredge-queen-3d/verify_dredge_queen.py")
...
def main() -> None:
    ...
    evidence = {
        ...
        "stateLaw": "working machine to intact gentle reprogramming; no damage or kill morph exists",
    ...
    assert abs(checked["bounds"]["size"][0] - 12.4) < 0.001
    ...
    for node, morph in EXPECTED.items():
        ...
        assert not morph.startswith("Damage_")

...
ARTIFACT_DIR = ROOT / "artifacts/town3d-general-store"
CHECKED = SOURCE_DIR / "general-store.glb"
REEXPORTED = ARTIFACT_DIR / "general-store-reexport.glb"
...
def main():
    ...
    bpy.ops.wm.open_mainfile(filepath=str(SOURCE_DIR / "general-store.blend"))
    bpy.ops.object.select_all(action="DESELECT")
    store = bpy.data.objects["GeneralStoreFullWrap"]
    ...
    (ARTIFACT_DIR / "reexport-evidence.md").write_text(
        "# General Store re-export evidence\n\n"
    ...
    assert checked["bounds"]["size"][0] <= 4.8 and checked["bounds"]["size"][2] <= 3.3

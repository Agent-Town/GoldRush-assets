...
ASSETS = {
    "water-ledger-office": {"mount": "claim_office", "footprint": [4.4, 3.2]},
    "greenkeeper": {"mount": "tavern", "footprint": [5.2, 3.4]},
    "ice-quarry-head": {"mount": "ice-quarry-pad", "footprint": [5.6, 3.8]},
    "weather-warden-spire": {"mount": "chapel", "footprint": [4.2, 3.2]},
    "canal-packet-boat": {"mount": "canal-water", "footprint": [5.2, 2.0]},
}
...
glb = load("e9_population_glb_contract", ROOT / "assets/pilots/dredge-queen-3d/verify_dredge_queen.py")
...
def reexport(asset_id: str, blend: Path, destination: Path) -> dict:
    ...
    bpy.ops.export_scene.gltf(
        filepath=str(destination.with_suffix("")), export_format="GLB", use_selection=True,
...
def main() -> None:
    ...
    for asset_id, assignment in ASSETS.items():
        ...
        temporary = Path(f"/tmp/{asset_id}-e9-reexport.glb")
        ...
        clearance = [round(footprint[0] - size[0], 6), round(footprint[1] - size[2], 6)]
        result = {
            ...
            "assignedMount": assignment["mount"],
            "mountEnvelope": footprint,
            "footprintClearance": clearance,
        }
        ...
        assert min(clearance) >= -0.001
        assert result["byteIdentical"] and all(result["semanticIdentical"].values())
        temporary.unlink(missing_ok=True)
    (RENDERS / "basin-population-asset-contract.json").write_text(json.dumps(evidence, indent=2) + "\n")
